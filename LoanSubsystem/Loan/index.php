...
//   Path A: Came from evergreen-marketing (has $_SESSION['user_id'] + $_SESSION['email'])
//   Path B: Logged in directly via loan system login.php (already has $_SESSION['user_email'])
// In both cases we resolve the user against loandb.users
// ─────────────────────────────────────────────────────────────────────────────
if (!isset($_SESSION['user_email'])) {
    if (isset($_SESSION['user_id']) && isset($_SESSION['email'])) {
        ...
        try {
            ...
            // Verify the session user actually exists in loandb.users
            $stmt = $pdo->prepare(
            ...
            if ($user) {
                ...
                // Not in loandb — redirect to login
                session_destroy();
...
<head>
  ...
  <style>
    ...
    .notification-header-text h3 { margin: 0; color: #003631; font-size: 18px; }
    .notification-item {
      background: white;
    ...
    .status-badge.active   { background: #2e7d32; color: white; }
    ...
    .notification-empty i { font-size: 64px; color: #ddd; margin-bottom: 20px; }
    .notification-empty p { font-size: 18px; color: #666; }
    .notification-timestamp { font-size: 13px; color: #888; font-style: italic; margin-top: 10px; }
    @keyframes fadeIn { from { opacity: 0; } to { opacity: 1; } }
    @keyframes slideDown {
      from { transform: translateY(-100px); opacity: 0; }
      to   { transform: translateY(0);      opacity: 1; }
    }
    .notification-modal-body::-webkit-scrollbar { width: 10px; }
    .notification-modal-body::-webkit-scrollbar-track { background: #f1f1f1; border-radius: 10px; }
    .notification-modal-body::-webkit-scrollbar-thumb { background: #003631; border-radius: 10px; }
    .notification-modal-body::-webkit-scrollbar-thumb:hover { background: #005a4d; }
    .loan-card.disabled {
      opacity: 0.5;
...
<section id="home" class="hero">
  ...
  <div class="hero-image">
    <img src="pictures/landing_page.png" alt="Apply for a Loan Easily">
  </div>
</section>
...
<section id="loan-services" class="loan-services-wrapper">
  ...
  <div class="loan-cards">
    ...
    <div class="loan-card" onclick="window.location.href='Loan_AppForm.php?loanType=Car%20Loan'">
      <img src="pictures/carloan.png" alt="Auto Loan">
      <div class="loan-card-content">
    ...
    <div class="loan-card" onclick="window.location.href='Loan_AppForm.php?loanType=Multi-Purpose%20Loan'">
      <img src="pictures/mpl.png" alt="Multipurpose Loan">
      <div class="loan-card-content">
...
<footer>
  <div class="footer-container">
    <div class="footer-top-columns">
      ...
      <div class="footer-col">
        <h3>Contact Us</h3>
        <p class="contact-item"><i class="fas fa-phone-alt"></i> 1-800-EVERGREEN</p>
        <p class="contact-item"><i class="fas fa-envelope"></i> support@evergreenbank.com</p>
    ...
    <div class="footer-bottom-bar">
      ...
      <div class="legal-links">
        ...
        <a href="#">Terms and Agreements</a><span class="separator">|</span>
        <a href="#">FAQs</a><span class="separator">|</span>
        <a href="#">About Us</a>
      </div>
    ...
    try {
      ...
      loans.forEach((loan) => {
        ...
        if (loan.status === 'Rejected' && loan.rejected_at) {
          allNotifications.push({
            ...
            pdf_path: loan.pdf_rejected || null
          });
        }
      });
      allNotifications.sort((a, b) => new Date(b.timestamp) - new Date(a.timestamp));

      // ── Disable loan cards if pending application exists ─────────────────
      ...
      if (hasPendingLoan) {
        ...
        loanCards.forEach(card => {
          ...
          let urlLoanType = loanType === 'Housing Loan'      ? 'Home Loan'
                          : loanType === 'Multipurpose Loan' ? 'Multi-Purpose Loan'
                          : loanType;
      ...
      loans.sort((a, b) => {
        const getDate = l => new Date(
          l.status === 'Active'    && l.approved_at  ? l.approved_at  :
          l.status === 'Rejected'  && l.rejected_at  ? l.rejected_at  :
          l.status === 'Approved'  && l.approved_at  ? l.approved_at  :
          l.created_at || 0
        );
      ...
      let activeCount = 0, pendingCount = 0, closedCount = 0,
          approvedCount = 0, rejectedCount = 0, earliestDueDate = null;
      ...
      loans.forEach((loan) => {
        ...
        if (loan.status === 'Active' && loan.next_payment_due) {
          nextPayment = new Date(loan.next_payment_due).toLocaleDateString('en-US', { year:'numeric', month:'short', day:'numeric' });
        } else if (loan.status === 'Approved') {
          if (loan.approved_at) {
            const claimDate = new Date(loan.approved_at);
            claimDate.setDate(claimDate.getDate() + 30);
            nextPayment = 'Claim by: ' + claimDate.toLocaleDateString('en-US', { year:'numeric', month:'short', day:'numeric' });
          } else {
...
function openNotificationModal() {
  ...
  if (allNotifications.length === 0) {
    ...
    allNotifications.forEach((notif) => {
      ...
      const formattedDate = new Date(notif.timestamp).toLocaleDateString('en-US', {
      ...
      if (notif.type === 'approved') {
        ...
        importantDate      = d.toLocaleDateString('en-US', { year:'numeric', month:'long', day:'numeric' });
        importantDateLabel = 'Claim Deadline';
      } else if (notif.type === 'active' && notif.next_payment_due) {
        importantDate      = new Date(notif.next_payment_due).toLocaleDateString('en-US', { year:'numeric', month:'long', day:'numeric' });
        importantDateLabel = 'Next Payment Due';
      }

      const remarksText = notif.type === 'rejected'
        ? (notif.rejection_remarks || '')
        : (notif.remarks || '');
...
function generatePDF(loanId, type, btn) {
  ...
  // Use XMLHttpRequest instead of fetch — avoids any browser redirect-following
  // that can silently turn a 302 JSON endpoint into an HTML page
  ...
  xhr.onload = function() {
    ...
    // Always log so developer can see the exact server output
    ...
    // If response starts with < it is HTML (redirect / PHP error page)
    if (trimmed.charAt(0) === '<') {
      var phpErr = trimmed.match(/(?:Fatal error|Parse error|Warning|Notice)[^<]{0,300}/i);
      var msg = phpErr
        ? phpErr[0].trim()
        : 'Server returned HTML instead of JSON (HTTP ' + xhr.status + ').\nCheck F12 > Console for the full response.';
      alert('PDF Error:\n' + msg);
      btn.innerHTML = orig;
      btn.disabled  = false;
      return;
    ...
    if (!data.success) {
      alert('PDF Error: ' + (data.error || 'Unknown error'));
      btn.innerHTML = orig;
      btn.disabled  = false;
      return;
    }

    // Success — save path to DB then swap button to download link
    ...
    xhr2.onload = function() {
      ...
      btn.outerHTML = '<a href="' + url + '" class="download-btn" download>'
                    + '<i class="fas fa-download"></i> Download PDF</a>';
      allNotifications.forEach(function(n) {
        if (n.id === loanId && n.type === type) {
          n.pdf_path = 'uploads/' + data.filename;
        }
      });
    };
  ...
  xhr.onerror = function() {
    alert('PDF Error: Network request failed. Check your connection.');
    btn.innerHTML = orig;
    btn.disabled  = false;
  };
...
// Auto-scroll to dashboard after submission
